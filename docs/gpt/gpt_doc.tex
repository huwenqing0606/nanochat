...
Then apply RoPE to $Q,K$, apply RMSNorm to $Q,K$, finally apply $(1,2)$ transpose $(B,T,H,D)\ra (B,H,T,D)$ to $Q,K,V$, i.e., make head be batch dim, so that we can parallel process each attention head to the full batch;
...
-- \verb#wte#: word-to-embedding layer.

-- \verb#h#: tramsformer block layer.

-- \verb#lm_head#: language modeling head), head layer, embedding $\ra$ token logits.
...
$\bullet$ Naive autoregressive streaming inference: generate the next token through output logits.
