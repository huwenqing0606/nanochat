...
-- Number of kv heads $\text{n\_kv\_head}$ $|$ number of heads $\text{n\_head}$. 

-- self-attention. Input $x$ has shape $(B,T,\_)$, then $Q, K, V$ are from input $x$, i.e., $Q=W_Q^T X$ etc. They are re-shaped into shape $(B,T, \text{n\_head}, \text{head\_dim})$ for $Q$ and $(B,T, \text{n\_kv\_head}, \text{head\_dim})$ for $K, V$, respectively. 
Then apply RoPE to $Q,K$, apply RMSNorm to $Q,K$, finally apply $(1,2)$ transpose $(B,T,H,D)\ra (B,H,T,D)$ to $Q,K,V$;
